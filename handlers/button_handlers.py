...
async def sos(message: Message):
    await message.answer(
        "@msibot_bot botining <a href=\"https://t.me/all_nc\">TAXIDASUPPORT</a> bilan bog'lanish",
        # noqa
...
async def start(message: Message):
    photo = FSInputFile(r'C:\Py\taksi_bot\images\taksi.jpeg')
...
async def support(message: Message):
    await message.answer(
        "@msibot_bot botining <a href=\"https://t.me/all_nc\">TAXIDASUPPORT</a> bilan bog'lanish", # noqa
        reply_markup=back_to_main.as_markup(resize_keyboard=True)
    )
...
async def need_client(message: Message, state: FSMContext):
    ...
    await message.answer('❓ Moshinangiz turini (markasini) tanlang:', reply_markup=cars_button.as_markup(resize_keyboard=True)) # noqa
...
async def start(message: Message, state: FSMContext):
    if message.text == '✅ Tasdiqlayman':
        ...
        if data['type'] in ("Yetib borish", 'Yetkazib berish'):
            ...
            await create_active_client([user['id'], data['type'], 'waiting', data['price'], from_, to, data['passenger_count'], additional_info]) # noqa
            await message.answer("Haydovchini kuting...", reply_markup=cancel_driver.as_markup(resize_keyboard=True)) # noqa
        else:

            await message.answer("Yo'lovchini kutish faollashtirildi ♻️", reply_markup=cancel_client.as_markup(resize_keyboard=True)) # noqa
    else:
        await message.answer("E'lon yuborish rad etildi", reply_markup=main_menu.as_markup(resize_keyboard=True)) # noqa
    await state.clear()
